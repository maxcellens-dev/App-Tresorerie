              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 2 }} keyboardShouldPersistTaps="handled">
                {Array.from(new Set(CATEGORY_ICON_GLOSSARY)).map((ic) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={ic}
                      // Grille d'icônes : le seul libellé qui distingue une cellule d'une autre est
                      // le nom de l'icône. `accessibilityState` porte la sélection courante.
                      accessibilityRole="button"
                      accessibilityLabel={`Icône ${ic}`}
                      accessibilityState={{ selected: active }}
                      style={[styles.iconCell, active && { backgroundColor: COLORS.emerald + '22', borderColor: COLORS.emerald }]}
