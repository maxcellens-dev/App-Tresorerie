...
import { useAdsConfig, useSaveAdsConfig, type AdBanner } from '../../../hooks/useAdsConfig';
...
export default function AdminAds() {
  ...
  useEffect(() => { if (loaded && banners === null) setBanners(loaded.banners); }, [loaded]);
  ...
  const add = () => setBanners([...banners, { id: `ad_${Date.now()}`, label: 'Nouvelle bannière', text: '', url: '' }]);
  ...
  async function persist() {
    if (!banners) return;
    setMsg(null);
    try { await save.mutateAsync({ banners }); setMsg('Enregistré ✓'); }
